...
exports.postLogin = (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    return res.status(422).render("auth/login", { msg: "invalidemail", email: userLoginData.email });
  }
  User.findOne({ email: userLoginData.email }).then(
    userDataFromDB => {
      if (userDataFromDB) {
        bcrypt
          ...
          .then(doMatch => {
            if (doMatch) {
              ...
              res.render("auth/login", { msg: "wrongpassword", email: userLoginData.email });
            }
          })
          .catch(err => {
        ...
        res.render("auth/login", { msg: "noemailfound", email: userLoginData.email });
      }
    }
  );
};
        ...
        User.create(user).then(() => {
          res.sgMail.send({
            ...
            html: `<h1 style="text-align:center">
                      Welcome, `+ userRegData.firstName + userRegData.lastName + `
                  </h1>
...
exports.postResetPassword = (req, res, next) => {
  ...
  User.findOne({ email: passResetEmail }).then(userDataFromDB => {
    if (!userDataFromDB) {
...
exports.getPasswordResetWithToken = (req, res, next) => {
  if (req.session.passResetToken) {
    if (req.session.passResetToken === req.params.passwordResetToken) {
      const msg = req.query.msg;
      res.render('auth/new-password', { passwordResetToken: req.params.passwordResetToken, msg: msg });
    } else {
      res.render('/');
    }
  } else {
...
exports.postNewPassword = (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.redirect(`/resetpassword-token/${req.session.passResetToken}?msg=invaildpass`);
  }
  if (req.session.passResetToken) {
    if (req.session.passResetToken === req.body.passwordResetToken) {
      if (req.body.newPassword !== req.body.cNewPassword) {
        res.redirect(`/resetpassword-token/${req.session.passResetToken}?msg=passnomatch`);
      } else {
        bcrypt.hash(req.body.newPassword, 12).then(hashedPassword => {
          User.updateOne({ _id: req.session.userId_passReset }, { password: hashedPassword }).then(() => {
            req.session.destroy(() => {
