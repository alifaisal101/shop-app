...
const imageUrl = require("./../util/functions/imageUrl");
  ...
  Product.findById(productId)
    .then((productFromdb) => {
      if (!productFromdb) {
        ...
        User.updateOne({ _id: res.userData._id }, { cart: newCart })
          .then(() => {
            res.redirect("/cart");
          })
          .catch((err) => {
...
exports.getCart = (req, res, next) => {
  ...
  Product.find({ _id: { $in: cartItemsIDs } })
    .then((cartProducts) => {
      let totalPrice = 0;
      for (let i = 0; i < cartProducts.length; i++) {
        for (let y = 0; y < res.userData.cart.items.length; y++) {
          if (
            `${cartProducts[i]._id}` ===
            `${res.userData.cart.items[y].productId}`
          ) {
...
exports.editQuantity = (req, res, next) => {
  ...
  User.updateOne({ _id: res.userData._id }, { cart: res.userData.cart })
    .then(() => {
      res.redirect("/cart");
    })
    .catch((err) => {
    ...
    .then((cartProducts) => {
      const newOrderItems = {
        items: cartProducts.map((cartPorduct, index) => {
          ...
          for (let i = 0; i < res.userData.cart.items.length; i++) {
            if (
              `${res.userData.cart.items[i].productId}` === `${cartPorduct._id}`
            ) {
